...
\begin{definition}[Dual Space, cf.~\cite{Ryn2008} Definition 4.28]\label{DuaSpa}
...
\begin{definition}[Weak Convergence for Dual Spaces, cf.~\cite{Ryn2008} Definition 5.68]\label{WeakCon}
...
\begin{definition}[Support, cf.~\cite{San2015} Definition 1.14]\label{Supp}
...
\begin{definition}[Atoms, adapted from~\cite{Bog2007} Definition 1.12.7]\label{Atoms}
...
\section{Kantorovich Relaxation}\label{KantRelax}
...
\begin{definition}[Transport Plans]\label{TransPlans}
...
\begin{definition}[Kantorovich Problem]\label{KanProb}
...
\begin{definition}[Weak Convergence for Measures, cf.~\cite{Bog2007}, Definition 8.1.1]\label{WeakConMeas}
...
\begin{lemma}[Taken from~\cite{San2015}, Lemma 1.31]\label{CompWeakConv}
	Consider on a compact metric space $X$, endowed with a probability $\rho \in \PM{X}$, a sequence of partitions $G_n$, each $G_n$ being a family of disjoint subsets $C_{i, n}$ such that $\bigcup\limits_{i \in I_n} C_{i, n} = X$ for every \NinN. Suppose that $\text{size}(G_n) := \max\limits_{i \in I_n} \text{diam}(C_{i, n})$ tends to $0$ as \Ninf{} and consider a sequence of probability measures $\rho_n$ on $X$ such that, for every \NinN{} and $i \in I_n$, we have $\rho_n(C_{i, n}) = \rho(C_{i, n})$. Then \weak{\rho_n}{\rho}.
	\todoinline{TODO:~Diameter definition!}
\end{lemma}
...
With the density of the induced transport plans shown, we can now conclude that the two innfima of $J$ and $K$ do coincide:

\begin{theorem}[Infima of J and K are the Same, adapted from~\cite{San2015}, Theorem~1.33]\label{InfCoincide}
	Let $\Omega \subset \R^d$ be a compact subset, $\mu, \nu \in \PM{\Omega}$ be two measures on $\Omega$ with $\mu$ being atomless, and $\map[c]{\Omega \times \Omega}{\RZero \cup \{ \infty \}}$ be a continuous cost function.
\end{theorem}

\begin{proof}
	This proof was adapted from~\cite{San2015}, Proof of Theorem~1.33 and the Memo on relaxations Box~1.10. \\
	\todoinline{TODO:~Combine relaxation definition (\cite{San2015}, Memo 1.10) and proof of theorem.}
\end{proof}

\todoinline{TODO:~Dual problem and existence of optimal solutions}