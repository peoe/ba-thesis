...
\begin{theorem}[Taken from~\cite{San2015}, Theorem 1.4]\label{KPAdmit}
	asfd
\end{theorem}
...
We have now seen, that both (KP) and (DP) admit solutions, but up to now the relation between these solutions has only been $\sup \text{(DP)} \le \inf \text{(KP)}$. Our aim is to now show $\inf \text{(KP)} \le \sup \text{(DP)}$. For this purpose, there are a few denominations we need to introduce:
