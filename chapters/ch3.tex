...
Note: Sequential compactness and compactness are the same on metric spaces. To see the set of transport plans as a compact set in a metric space, one may consider the linear space of all bounded and signed measures with the total variation norm $\Vert \mu \Vert := \sup (|\mu(A)| + |\mu(A^C)|)$ with the supremum being considered over all elements of the underlying $\sigma$-algebra. This 

\todoinline{Alternative: Prokhorov~\ref{Prok}}
...
\todoinline{TODO:~start with c-CM and continue onto $dual = primal$}
...
\begin{lemma}[Adapted from~\cite{San2015}, Theorem~1.37]\label{c-CM-c-ConcExist}
	Let $\Omega \subseteq X \times Y$ be a $c$-CM set with $\Omega \neq \emptyset$ and \map[c]{X \times{} Y}{\R} be an arbitrary function. Then there exists a $c$-concave function $\map[\varphi]{X}{\R \cup \{ - \infty \}}$ such that
...
The problem we now face, is that on arbitrary spaces our $c$ will no longer be uniformly continuous or bound from above (the bound from below is always satisfied, when we consider ``practical'' cost functions mapping to $\RZero \cup \{ \infty \}$). It is possible to avoid the constraints on uniform continuity and an upper bound. After these next results we will finish this part and continue towards transport plan and transport map estimation.
