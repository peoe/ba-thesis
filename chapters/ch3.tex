...
\begin{proof}
	We consider an increasing sequence ${(\varphi_n, \psi_n)}_{\NinN}$ for (DP). As this sequence must be feasible, we have $\forall x \in X, y \in Y: \psi_n(y) \le c(x, y) - \varphi_n(x)$ and note that
	\[ \int\limits_Y \psi_n~\D[\nu] \le \int\limits_Y \inf\limits_{x \in X} c(x, y) - \varphi_n(x)~\Dx[y]{\nu} = \int\limits_Y \varphi_n^c~\D[\nu]. \]
	Thus it will suffice to consider an increasing sequence ${(\varphi_n, \varphi_n^c)}_{\NinN}$. The same argument for $\varphi_n^c$ results in an increasing sequence ${(\varphi_n^{c\bar{c}}, \varphi_n^c)}_{\NinN}$. As 
	\[\forall x \in X: \varphi^{c\bar{c}}(x) = \inf\limits_{y \in Y} c(x, y) - \varphi^c(y), \]
	further iteration cannot increase the sequence indefinitely. This already means that for an arbitrary pair $(\varphi, \psi)$ to be a solution, it is neccessary for $\varphi^c = \psi$ and $\psi^{\bar{c}} = \varphi$ to hold, i.e.~they must be $\bar{c}$-concave and $c$-concave respectively, since the integrals otherwise could be enlarged by considering the $c$- and $\bar{c}$-transforms of said solution.
\end{proof}
