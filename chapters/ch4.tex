...
\begin{definition}[Regularized Problems; adapted from~\cite{Cla2021}, the beginning of Section~3 and Proposition~3.1]\label{RegProbs}
	...
	Just as in Definition~\ref{MongeProb},~\ref{KanProb} and~\ref{KanDual}, we will use the abbreviations (RP), $\inf \text{(RP)}$, (PD) and $\sup \text{(PD)}$ for the problems and their solutions respectively. Further, by~\cite{Cla2021}, Proposition~3.1, if $\sup \text{(PD)}$ is finite, (RP) admits a minimizer.
\end{definition}
...
As we have previously seen, the regularization term is finite if and only if $\gamma \in \LLL$. By considering (RP) over \LLL{} instead, we can also derive a dual problem over $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$. Following the procedure from~\cite{Cla2021}, Section~4, we apply
\[ \Phi(s) := \begin{cases}
	\infty & , s < 0 \\
	s & , 0 \le s \le 1 \\
	\exp{s - 1} & , else
\end{cases} \text{, and } \Psi(s) := \log \big( \Phi(s) \big). \]
to get
\[ \phi = \varepsilon \log \big( \Phi(u_1) \big) = \varepsilon \Psi(u_1) \text{ and } \psi = \varepsilon \log \big( \Pi(u_2) \big) = \varepsilon \Psi(u_2). \]
