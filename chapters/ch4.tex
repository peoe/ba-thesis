...
as an approximate value of transport plan for (RP) at a predetermined $(x, y)$. Resubstituting once more with $\exp\left( \frac{\varphi^N(x)}{\varepsilon} \right) = \Phi\big( u_1^N(x) \big)$ as well as $\exp\left( \frac{\psi^N(y)}{\varepsilon} \right) = \Phi\big( u_2^N(y) \big)$, we obtain
\[ \gamma_N(x, y) = \exp\left( \frac{\varphi^N(x) + \psi^N(y) - c(x, y)}{\varepsilon} \right). \]
...
\begin{algorithm}\label{OTPlanEstAlg}
	...
	\While{$\text{\normalfont{}not converged}$}{
		...
		$\varphi_{n + 1} := \varphi_n + \rho \sum\limits_{i, j = 1}^p \Big( \nabla \varphi_n(x_i) + \partial_{\varphi_n} F_\varepsilon\big( \varphi_n(x_i), \psi_n(y_j) \big) \nabla\varphi_n(x_i) \Big)$\;
		$\psi_{n + 1} := \psi_n + \rho \sum\limits_{i, j = 1}^p \Big( \nabla \psi_n(x_i) + \partial_{\psi_n} F_\varepsilon\big( \varphi_n(x_i), \psi_n(y_j) \big) \nabla\psi_n(y_j) \Big)$\;
	}
\end{algorithm}

The most apparent advantage of this approach is that, in the case of $\mu$ and $\nu$ both being discrete, the stochastic gradient descent method converges at a rate of $\mathcal{O}\left( \frac{1}{\sqrt{k}} \right)$, where $k$ is the iteration number, but only requires a cost of $\mathcal{O}(p^2)$ per iteration. In the case of both measures being continuous, the cost per iteration remains $\mathcal{O}(p^2)$, but convergence can only be ensured to a stationary point.

\todoinline{TODO:~plan estimation (\cite{Seg2018}, Theorem 3.1 and Algorithm 1)}
