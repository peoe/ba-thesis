\chapter{Mapping Estimation}\label{DualPlanEst}
...
There are multiple ways that regularization can take place, however we will focus on \textit{entropic regularization}. This form of regularization deals with measurable functions with finite entropy, i.e.~for $f \in \MF{\Omega}{\R}$
...
Figure~\ref{RegImages} shows the effect of regularization. The software provided by\ \cite{PythonOT} was used to generate the solutions between a Gaussian and a uniform distribution. The upper left image shows the unregularized result and the remaining images show the results for $\varepsilon \in \{ 1, 0.1, 0.01, 0.001, 0.0001 \}$.

\todoinline{TODO:~add remark on how reg effects solutions (images! via pythonot.github.io)}
\begin{figure}
	...
	\caption{Test images}\label{RegImages}
\end{figure}
...
\begin{proof}
	...
	\todoinline{TODO:~write extensive proof of this lemma}
\end{proof}
