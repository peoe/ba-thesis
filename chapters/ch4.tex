...
We will first introduce regularized optimal transport. Regularization imposes an additional penalty on the objective function of an optimization problem. This can serve multiple purposes: regularizing an ill-posed problem can allow for an approximate solution, result in uniqueness of the solution or, especially in the case covered in this thesis, allow for duality results which net themselves for further applications.

The following duality result will then be used to obtain an optimal transport map via the barycentric projection. Due to the explicit computation of these transport solutions often being impractical for applications, it is necessary to estimate optimal transport plans and maps. This is traditionally done using the Sinkhorn algorithm, however this particular algorithm is not suited to handle large-scale problems. Hence, a different approach involving neural networks and stochastic mapping estimation is required.

The first sections on regularization and the corresponding regularized dual problem follow\ \cite{Cla2021} closely, while the last section is adapted from\ \cite{Seg2018}.
...
\[ \Phi_{\exp}(s) := \begin{cases}
	...
	\exp(s - 1), & s > 1
\end{cases} \]
and $\Lexp := \Orlicz{\Phi_{\exp}}$ with its corresponding Luxemburg norm $\Vert \cdot \Vert_{\Phi_{\exp}}$. As shown in~\cite{Cla2021}, Proposition~2.7, if $\Omega$ has finite Lebesgue measure (which it has by $\Omega \subseteq \R^{n_1 + n_2}$ being compact and thus bounded), then
\[ {(\LLL)}' = \Lexp. \]
Here, ${(\cdot)}'$ denotes the dual space in accordance with\ \cite{Ryn2008}, Definition 4.28.
...
As an immedtiate consequence, we can even avoid the notion of transport plans via the push-forward, and directly formulate the barycentric projection as a transport map.
...
Before we are able to establish large-scale mapping estimation for optimal transport problems, we need to investigate the dual of the regularized problem. This will occur in a similar way, as was done in Section~\ref{KanDual} for the dual of (KP).

\section{Regularized Duality}\label{RegDual}

As we have previously seen, the regularization term is finite if and only if $\gamma \in \LLL$. By considering (RP) over \LLL{} instead of \TP{\mu}{\nu}, we can also derive a dual problem over $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$. Following the argument put forth in~\cite{Cla2021}, Section~4, we define
\[ \Phi(s) := \begin{cases}
	\infty, & s < 0 \\
	s, & 0 \le s \le 1 \\
	\exp(s - 1), & s > 1
...
\begin{proof}
	...
	and show that it possesses a minimizer. The value taken by $B$ is finite for e.g.\ $u_1 \equiv u_2 \equiv 1$, allowing us to consider a minimizing sequence ${(u_1^n, u_2^n)}_{\NinN}$ with $(u_1^n, u_2^n) \in \Lexp[\Omega_1] \times \Lexp[\Omega_2]$ for all \NinN. By\ \cite{Cla2021}, Lemma~4.2, we can assume $\int \Phi_{\exp}(u_1^n)~\D[x] = 1$ without loss of generality. By applying\ \cite{Cla2021}, Lemma~4.4, we can observe that $\Vert u_2^n \Vert_{\Phi_{\exp}}$ is bounded. By applying the Theoreom of Banach-Alaoglu (\ref{Ban-Alao}), we obtain a weakly-* convergent subsequence $(u_1^{n_k}, u_2^{n_k})$. By\ \cite{Cla2021}, Lemma~4.5, $B$ is sequentially weakly-* lower semi-continuous on $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$, showing that the minimum is attained over $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$.
\end{proof}
...
\begin{proof}
	This proof was taken from the proof of Proposition~4.7 in~\cite{Cla2021}. The existence of a solution follows from Theorem~2.2 and Theorem~4.6.
...
As the final result of this section, the following corollary provides optimality conditions for solutions of (RD).
...
The final section of the second part will contain three algorithms used for mapping estimation. The first will make use of the preceding optimality conditions in Corollary~\ref{RegOptCond}. The second and the third algorithms will improve on the first to be applicable to large-scale problems. In particular, the dual solutions derived in this section will be estimated by the second algorithm, and a barycentric projection from Section~\ref{BaryProj} approximated for these estimates in the third algorithm.

\section{Stochastic Plan and Mapping Estimation}\label{StoPlanAndMapEst}

In this final section, the focus will be on the estimation of optimal transport solutions, that is both plans and maps. The first step is the Sinkhorn algorithm, which is a well-known approach. We will then go on to describe a method involving stochastic gradient descent, as has been detailed by\ \cite{Seg2018}.

We can use the optimality conditions from Corollary~\ref{RegOptCond} to derive the \textit{Sinkhorn algorithm}, as~\cite{Cla2021}, Remark~4.9, points out. The idea of this algorithm is twofold: apply equations\ \ref{OptCond1} and\ \ref{OptCond2} to iteratively approach optimal dual variables, and then use the last equation to obtain an estimated optimal transport plan.
...
This concludes the thesis, having stated two algorithms that produce an estimate optimal transport map for optimal transport problems.