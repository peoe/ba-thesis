...
The first sections on regularization follow\ \cite{Cla2021} closely, while the last section is adapted from\ \cite{Seg2018}.
...
In this final section, the focus will be on the estimation of optimal transport solutions, that is both plans and maps. The first step is the Sinkhorn algorithm, which is a well known approach. We will then go on to a method involving stochastic gradient descent, as described by\ \cite{Seg2018}.
...
In order to now state the first algorithm, we need to adapt the previous definition of $F_\varepsilon$ slightly to include the dual variables:
