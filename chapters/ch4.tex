...
\begin{proof}
	...
	and show that it possesses a minimizer. The value taken by $B$ for e.g.\ $u_1 \equiv u_2 \equiv 1$, allowing us to consider a minimizing sequence ${(u_1^n, u_2^n)}_{\NinN}$ with $(u_1^n, u_2^n) \in \Lexp[\Omega_1] \times \Lexp[\Omega_2]$ for all \NinN. By\ \cite{Cla2021}, Lemma~4.2, we can assume $\int \Phi_{\exp}(u_1^n)~\D[x] = 1$ without loss of generality. By applying\ \cite{Cla2021}, Lemma~4.4, we can observe that $\Vert u_2^n \Vert_{\Phi_{\exp}}$ is bounded. By applying the Theoreom of Banach-Alaoglu (\ref{Ban-Alao}), we obtain a weakly-* convergent subsequence $(u_1^{n_k}, u_2^{n_k})$. By\ \cite{Cla2021}, Lemma~4.5, $B$ is sequentially weakly-* lower semi-continuous on $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$, showing that the minimum is attained over $\Lexp[\Omega_1] \times \Lexp[\Omega_2]$.
\end{proof}
...
In this final section, the focus will be on the estimation of optimal transport solutions, that is both plans and maps. The first step is the Sinkhorn algorithm, which is a well-known approach. We will then go on to describe a method involving stochastic gradient descent, as described by\ \cite{Seg2018}.

We can use the optimality conditions in Corollary~\ref{RegOptCond} to derive the \textit{Sinkhorn algorithm}, as~\cite{Cla2021}, Remark~4.9, points out. The idea of this algorithm is twofold: apply the first two equations to iteratively approach optimal dual variables, and then use the last equation to obtain an estimated optimal transport plan.
...
\begin{definition}[Neural Network; adapted from\ \cite{Good2016}, Chapter~6, and\ \cite{Brag2020}, Section~6.2]
	...
	\end{itemize}
	The dimensions of these matrices, vectors and functions are chosen appropriately, such that for fixed $\NinN[d, l]$ the neural network $\mathcal{H}$ can be evaluated for an input vector $x \in \R^d$ and an output vector $y \in \R^l$ be obtained by applying the recursive formula
...
\begin{remark}[Taken from\ \cite{Brag2020}, Formulae~6.48--6.52]
	Typical \textit{activation functions} are
	\begin{itemize}
...
In order to now state the first algorithm, we need to adapt the previous definition of $F_\varepsilon$ in Definition~\ref{RegProbs} slightly to include the dual variables:
...
As mentioned above, the resulting dual variables take the form of vectors, when $\mu$ and $\nu$ are discrete, and are modelled by neural networks in the continuous case.
