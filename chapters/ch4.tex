...
\begin{proof}
	\todoinline{TODO:~Add outline why we only have to show the following and properly introduce our $T$! Also add notice for $K_g$ as the Lipschitz constant of $g$!}
	By \todo{Add reference to Villani, 2008, Corollary 9.3} there exists a Monge map between $\mu$ and $\nu$ due to $\mu$ being uniformly continuous on $\Omega_1$ and thus absolutely continuous with respect to the Lebesgue measure. Following the proof of~\cite{Seg2018}, Theorem~2, we only have to show that for any Lipschitz continuous function $g$ over $\Omega$
	...
	We apply the closed form of the barycentric projection as provided by~\cite{Seg2018} $\bar{\gamma}(x) = \gamma(x) Y$, with $Y := {(y_1, \dots, y_n)}^T$ and $\gamma(x) := (\gamma(x, y_1), \dots \gamma(x, y_n))$, to obtain
	...
	where $\Vert A \Vert_{\R^{d \times n}, 2} = \sigma_{\max}(A^T A)$, $\Vert A \Vert_{\R^{n \times n}, 2} = \sigma_{\max}(A^T A)$, $\gamma_n^{\varepsilon_n} = {\big( \gamma_n^{\varepsilon_n}(x_i, y_j) \big)}_{i, j = 1}^n$, $\gamma_n$ in a similar fashion, and Cauchy-Schwarz is used on the last inequality. By the same result by \todo{add reference to Cominetti and San Martin, 1994} as in the proof of Theorem~\ref{RelatRegSolOrigSolPlan}, we can now find $M_{c_n, \mu_n, \nu_n}, \lambda_{c_n, \mu_n, \nu_n} > 0$ such that
...
\begin{corollary}[Taken from~\cite{Seg2018}, Corollary~1]\label{RelatRegOrigCor}
	With the same assumptions as for Theorem~\ref{RelatRegSolOrigSolMap}, there exists a subsequence $(n_k)_{\NinN[k]}, \NinN[n_k]$ for all \NinN[k], such that \weak[]{\push[\big(\bar{\gamma}^{\varepsilon_{n_k}}_{n_k}\big)]{\mu_{n_k}}}{\nu} for \Ninf.
\end{corollary}
